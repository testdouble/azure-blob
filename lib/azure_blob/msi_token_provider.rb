...
module AzureBlob
  class MsiTokenProvider
    ...
    def initialize(expiration_threshold: 10.minutes, principal_id: nil)
      # TODO Decide if we are going to use the IMDS uri or the localhost URI
      @msi_identity_uri = URI.parse(IMDS_URI)
      params = {
    ...
    def get_new_token_from_imds
      ...
      response = Net::HTTP.get_response(msi_identity_uri, headers)
